...
class ConversationMemory:
    ...
    async def get_user_preferences_db(self, user_id: int) -> Dict:
        ...
        try:
            async with AsyncSessionLocal() as session:
                user = await session.execute(select(User.id, User.telegram_id).where(User.telegram_id == str(user_id)))
                user_row = user.first()
                if not user_row:
                    return {}
                user_id_db = user_row[0]
                prefs = await session.execute(select(UserPreference).where(UserPreference.user_id == user_id_db))
    ...
    async def update_user_preference_db(self, user_id: int, key: str, value: str) -> None:
        ...
        try:
            async with AsyncSessionLocal() as session:
                # Use basic user creation approach to avoid schema issues
                from sqlalchemy import text
                user = await session.execute(select(User.id, User.telegram_id).where(User.telegram_id == str(user_id)))
                user_row = user.first()
                if not user_row:
                    # Create basic user record using raw SQL
                    result = await session.execute(text(
                        ...
                        'full_name': f'Telegram User {user_id}',
                        'is_active': True,
                        'created_at': datetime.utcnow()
                    })
                    user_id_db = result.scalar()
                    await session.commit()
                else:
                    user_id_db = user_row[0]
                
                pref = await session.execute(select(UserPreference).where(UserPreference.user_id == user_id_db, UserPreference.key == key))
                pref_obj = pref.scalars().first()
                if pref_obj:
                    pref_obj.value = value
                else:
                    pref_obj = UserPreference(user_id=user_id_db, key=key, value=value)
