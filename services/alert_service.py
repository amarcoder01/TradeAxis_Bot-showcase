...
class AlertService:
    ...
    def __init__(self, market_service: MarketDataService, notification_callback: Optional[Callable[[int, str], Awaitable[None]]] = None):
        ...
        self.check_interval = 30  # Check every 30 seconds for notifications
    ...
    async def add_alert(self, telegram_user_id: int, symbol: str, condition: str, target_price: float, alert_type: str = "price") -> Dict:
        ...
        try:
            ...
            async with AsyncSessionLocal() as session:
                if not user_obj:
                    # Ensure user exists - only select fields that exist in database
                    user = await session.execute(select(User.id, User.telegram_id).where(User.telegram_id == str(telegram_user_id)))
                    user_row = user.first()
                    if not user_row:
                        # Create basic user record using raw SQL to avoid model issues
                        from sqlalchemy import text
                        result = await session.execute(text(
                            "INSERT INTO users (telegram_id, email, username, hashed_password, full_name, is_active, created_at) VALUES (:telegram_id, :email, :username, :hashed_password, :full_name, :is_active, :created_at) RETURNING id"
                        ), {
                            'telegram_id': str(telegram_user_id),
                            'email': f'user_{telegram_user_id}@telegram.com',
                        ...
                        # Create a minimal user object for caching
                        class MinimalUser:
                            def __init__(self, user_id):
                                self.id = user_id
                        user_obj = MinimalUser(user_id)
                    else:
                        # Create minimal user object from existing data
                        class MinimalUser:
                            def __init__(self, user_id):
                                self.id = user_id
                        user_obj = MinimalUser(user_row[0])
    ...
    async def start_monitoring(self):
        ...
        logger.info("✅ Alert monitoring service started successfully")
    ...
    async def _check_single_alert_with_price(self, alert: Alert, current_price: float, session):
        ...
        try:
            ...
            if triggered:
                ...
                # Invalidate user alerts cache
                user = await session.execute(select(User.telegram_id).where(User.id == alert.user_id))
                user_telegram_id = user.scalar()
                if user_telegram_id:
                    alerts_cache_key = f"user_alerts_{user_telegram_id}"
                    performance_cache.delete(alerts_cache_key)
        ...
        try:
            ...
            if cached_stats is None:
                async with AsyncSessionLocal() as session:
                    ...
                    total_users = await session.execute(select(User.id))
